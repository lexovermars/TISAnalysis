...
def main(argv):
	...
	try:
		...
		print 'score_create_new_annotation.py -i <inputpttfile> -o <outputfile>'
		sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'score_create_new_annotation.py -i <inputfile> -o <outputfile>'
			sys.exit()
		elif opt in ("-i", "--ifile"):
			inputfile = arg
		elif opt in ("-o", "--ofile"):
...
def determine_topscores(sorted_pca_scores):
	...
	for orf in top_scores.keys():
		...
		#Write new ptt file
		ptt_data = ptt_dict[orf]
		#Replace start position by new one
		if ptt_data[1] == "+":
			candidate_pos += 1
			location_string = str(int(candidate_pos))+".."+ptt_data[0].split("..")[1]
		else:
			location_string = ptt_data[0].split("..")[0]+".."+str(int(candidate_pos))
		ptt_data[0] = location_string
		ptt_line = "\t".join(ptt_data)
		output_new_ptt.write(ptt_line)
...
if __name__ == "__main__":
	...
	# Open new ptt file
	output_new_ptt = open("output/"+outputfile_name+"_adjusted_ptt.txt","w")
	determine_topscores(sorted_pca_scores)

	output.close()
	output_new_annotation.close()
	output_new_ptt.close()
